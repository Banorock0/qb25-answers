...
mito_med <- dt_mito %>% group_by(is_mitochondrial) %>% filter(Liver > 1) %>% summarize(mean = mean(Liver), median(Liver))

# Interpretation here as a comment
# Nuclear: mean = 34.7, median = 6.51
# Mito: mean = 18648.8, median = 18805.5
# The mitochondrial expression is much higher than the nuclear expression, and the distribution is relatively even skewed, indicating that most mitochondrial genes have a high level of expression around the mean. The nuclear expression is right skewed, indicating that the majority of nuclear genes have low expression with a few outliers with higher expression.
