...
# answer here
hist_l <- ggplot(dt, aes(x = Liver)) +
  geom_histogram(breaks = seq(0, 100, 2)) +
  xlab("Liver Expression (TPM)")

hist_l
# The distribution is right skewed. This indicates that most genes have low expression, with few highly expressed genes. This is likely true across most tissues because the expression levels include genes that are highly expressed only in specific tissues, so the same genes will be close to 0 expression in the other tissues.

hist_b <- ggplot(dt, aes(x = Bladder)) +
  geom_histogram(breaks = seq(0, 100, 2)) +
  xlab("Bladder Expression (TPM)")
...
# answer here
den_l <- ggplot(dt, aes(x = Liver)) +
  geom_density() +
  xlab("Liver Expression (TPM)")

den_l

den_llog <- ggplot(dt, aes(x = log10(Liver+1))) +
  geom_density() +
  xlab("Log Liver Expression log10(TPM)")
...
# answer here
dt_HLV <- dt %>% rename(HLV = "Heart - Left Ventricle")

scat_lh <- ggplot(dt_HLV, aes(x = log10(Liver+1), y = log10(HLV+1))) + 
  geom_point(size = 1, alpha = 0.2) +
  geom_abline(slope = 1, color = "red") +
  xlab("Liver Expression log10(TPM)") +
  ylab("Heart - Left Ventricle log10(TPM)")
...
# answer here
dt_mito <- dt_HLV %>% mutate(is_mitochondrial = str_starts(Description,"MT-")) # citation: Brooke W

scat_mito <- ggplot(dt_mito, aes(x = log10(Liver+1), y = log10(HLV+1), color = is_mitochondrial)) + 
  geom_point(size = 1, alpha = 0.2) +
  geom_abline(slope = 1, color = "red") +
  xlab("Liver Expression log10(TPM)") +
  ylab("Heart - Left Ventricle log10(TPM)") +
...
```{r}
focal_tissues <- c("Liver", "Testis", "Whole Blood", "HLV", "Brain - Cortex")
```
...
hist_5 <- ggplot(dt_long, aes(x = TPM)) +
  geom_histogram(breaks = seq(0, 100, 2)) +
  facet_grid(.~Tissue)
...
hist_10 <- ggplot(dt_long, aes(x = TPM)) +
  geom_histogram(breaks = seq(0, 100, 2)) +
  facet_grid(is_mitochondrial ~ Tissue, scales = "free_y", labeller = labeller(is_mitochondrial = c("FALSE" = "Nuclear","TRUE" = "Mitochondrial")))
